...
__global__ void init(int n, float *x, float *y)
{
    ...
    {
        printf("\n TId[%d] ", threadIdx.x);
        printf(" WId[%zu] ", warp_id);          // Corrected to %zu
        printf(" LId[%d] ", lane_id);           // Corrected to %d for int
        printf(" WperG[%zu] ", warps_per_grid); // Corrected to %zu
        printf(" wTot[%zu] ", warp_total);      // Corrected to %zu
