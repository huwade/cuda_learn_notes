...
int main()
{
    ...
    { // depth-first launch
        cudaMemcpyAsync(d_x + FIXME * i, h_x + FIXME * i, (FIXME) * sizeof(ft), cudaMemcpyHostToDevice, streams[i % num_streams]);
        gaussian_pdf<<<(FIXME + 255) / 256, 256, 0, streams[i % num_streams]>>>(d_x + FIXME * i, d_y + FIXME * i, 0.0, 1.0, FIXME);
